...
public class JoinValidator implements Validator<RequestJoin>, RequiredValidator,
    ...
    public void check(RequestJoin form) {
        ...
        boolean termsAgree = form.isTermsAgree();
          ...
          checkRequired(confirmPassword, new AlertException("비밀번호를 입력하세요.", status));
        ...
        //이메일 중복 여부 체크 - 이미 가입된 회원인지 여부
         checkTrue(mapper.exists(email)== 0L, new AlertException("이미 가입된 이메일" +
                 "입니다.", status));

        // 비밀번호 자리수는 8자리 이상
checkTrue(password.length() >= 8, new AlertException("비밀번호는 8자리 이상" +
        "입력하세요.", status));

        // 비밀번호와 비밀번호 확인의 일치 여부
        checkTrue(password.equals(confirmPassword), new AlertException("비밀번호가 일치하지" +
                "않습니다.", status));
